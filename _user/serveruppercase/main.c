...
{
	printf("%s: len=%d", header, (unsigned long)len);
...
static ssize_t server_handleWrite(oid_t *oid, const void *data, size_t len, struct Buffer * buffer)
{
	...
	printf("Write from oid %u:%u of %zu bytes",
		(unsigned)oid->port, (unsigned)oid->id, len);
	...
	for(; i < len; ++i)
	{
		...
		if( isalpha(((char *) data)[i]) )
		{
			if (buffer -> len == BUFFER_SIZE)
				break;
			buffer->data[(buffer->len)++] = toupper(((char *)data)[i]);
		}
	}
...
int main(void)
{
	...
	if (portCreate(&oid.port) < 0) {
		fprintf(stderr, "serveruppercase: portCreate failed\n");
		return EXIT_FAILURE;
	}
	...
	if (create_dev(&oid, "serveruppercase") < 0) {
		fprintf(stderr, "serveruppercase: create_dev failed\n");
		return EXIT_FAILURE;
	}
